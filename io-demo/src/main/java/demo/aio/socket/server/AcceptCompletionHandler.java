...
public class AcceptCompletionHandler extends Base implements CompletionHandler<AsynchronousSocketChannel, AsyncServerHandler> {
	...
	public void completed(AsynchronousSocketChannel result, AsyncServerHandler attachment) {
		 ...
		 * 因此我们要在第一个客户端连接结果的回调函数中再次开启一个accept方法以接收第二个客户端连接
		 * ，递归调用，就可以支持accept无数个客户端连接了。
		 */
		 ...
		 * 
		 * 其中attachment依然作为其回调时的入参：读数据的时候，是通过ByteBuffer容器，无论是数据源还是结果存放，
		 * 因此attachment也应该传入一个ByteBuffer对象
		 * 
		 * 最后一个参数为异步读操作的回调函数。
		 */
